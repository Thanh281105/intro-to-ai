...
class AStarAgent(Agent):
    ...
    def __init__(self, player_id: int = 1, weights: Optional[CompetitiveWeights] = None, evaluator_type: str = 'new'):
    ...
    def choose_action(self, state, board, time_limit_ms: int = 1000, step_limit: int = 25) -> str:
        ...
        serial = count()
        open_pq = [(initial_h, 0, next(serial), start_state, None, 0)]
        best_g = {(my_pos, state.boxes, state.owners): 0}
        best_plan_first_action = None
        best_h_seen = initial_h
        ...
        while open_pq:
            ...
            f, g, _, (cur_pos, cur_boxes, cur_owners, c_step), first_action, depth = heapq.heappop(open_pq)
            state_key = (cur_pos, cur_boxes, cur_owners)
            ...
            if cur_h < best_h_seen and first_action is not None:
                best_h_seen = cur_h
                best_plan_first_action = first_action
                if cur_h <= -self.weights.terminal_win:
                    break
            ...
            for action, (dr, dc) in DIRECTIONS.items():
                ...
                if ng < best_g.get(nxt_state_key, float('inf')):
                    best_g[nxt_state_key] = ng
